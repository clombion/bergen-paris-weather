...
import { tags, nodes } from "./markdoc/tags.ts";
import i18nData from "./i18n/dynamic.ts";
...
function buildI18nJson(lang: string): string {
  const strings = (i18nData as Record<string, Record<string, string>>)[lang];
...
function buildLangToggle(lang: string, isAbout: boolean): string {
  const enActive = lang === "en" ? ' class="active"' : "";
  const frActive = lang === "fr" ? ' class="active"' : "";

  let enHref: string;
  let frHref: string;
  let aboutHref: string | null;
  if (isAbout) {
...
function buildToc(ast: any): string {
  const links: string[] = [];
  const idPattern = /\s*\{#([^}]+)\}\s*$/;
...
const labels: Record<string, Record<string, string>> = {
  en: {
    footer:
      'A project by <a href="https://linkedin.com/in/cedriclombion" style="color:var(--muted)">C\u00e9dric Lombion</a>. Data: <a href="https://open-meteo.com/" style="color:var(--muted)">Open-Meteo</a> (CC BY 4.0). Code: MIT. <a href="https://github.com/clombion/bergen-paris-weather" style="color:var(--muted)">GitHub</a>.',
    backTop: "Back to top",
  ...
  fr: {
    footer:
      'Un projet de <a href="https://linkedin.com/in/cedriclombion" style="color:var(--muted)">C\u00e9dric Lombion</a>. Donn\u00e9es\u00a0: <a href="https://open-meteo.com/" style="color:var(--muted)">Open-Meteo</a> (CC BY 4.0). Code\u00a0: MIT. <a href="https://github.com/clombion/bergen-paris-weather" style="color:var(--muted)">GitHub</a>.',
...
function getDataBase(outputPath: string): string {
...
function getStylesBase(outputPath: string): string {
  const outputDir = path.dirname(outputPath);
...
for (const page of pages) {
  ...
  if (ast.attributes && ast.attributes.frontmatter) {
    try {
      frontmatter = yaml.load(ast.attributes.frontmatter as string) || {};
    } catch (e: any) {
  ...
  if (errors.length) {
    ...
    errors.forEach((e: any) =>
      console.error(`    line ${e.lines?.[0] || "?"}: ${e.error?.message || e.message || JSON.stringify(e)}`),
    );
    skipped++;
    continue;
  }
  ...
  const title =
    ...
    (isAbout
      ? page.lang === "fr"
        ? "\u00c0 propos \u2014 Bergen vs Paris"
        : "About \u2014 Bergen vs Paris"
      : page.lang === "fr"
        ? "Bergen vs Paris \u2014 La surprise hivernale"
        : "Bergen vs Paris \u2014 The Winter Surprise");
