...
Parti d'une intuition personnelle : « les climats de Bergen et Paris sont similaires ». Nous l'avons déclinée en quatre hypothèses testables :
...
Six sources de données ont été étudiées (Open-Meteo, Meteostat, Frost API, Météo-France, Visual Crossing, NOAA GSOD). Open-Meteo était la seule à proposer les six variables requises (temp moy/min/max, précipitations, neige, ensoleillement) pour les deux villes, sans données manquantes ni clé API.
...
- **Exhaustivité** : séquences de dates continues, aucune valeur manquante
- **Qualité** : aucune température hors des records climatiques plausibles, aucune précipitation négative, aucune série anormale de valeurs constantes
...
De nombreux graphiques incluent une petite composante interactive pour garder l'attention : filtrer la carte thermique par niveau de similarité, parcourir les années sur le graphique quotidien, ou choisir une capitale européenne sur la carte des distances aériennes pour rendre concret le fait que la Norvège est un pays très long.
